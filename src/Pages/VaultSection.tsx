...
type MangaName = keyof typeof mangaData;
type Panel = (typeof mangaData)[MangaName][number];
...
const MangaVault = () => {
  ...
  const handleMangaClick = (mangaName: MangaName) => {
        ...
        <div className="px-6 py-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {Object.keys(mangaData).map((mangaName) => (
              <button
                key={mangaName}
                onClick={() => handleMangaClick(mangaName as MangaName)}
                className={`px-4 py-2 rounded-lg font-medium transition-all duration-200 ${
      ...
      <div className="p-6 text-center font-comicneue">
        <div className="mb-6">
          ...
          <p className="text-gray-400">
            {filteredPanels.length} panel
            {filteredPanels.length !== 1 ? "s" : ""} available
          </p>
        </div>
          ...
          >
            {filteredPanels.map((panel) => (
              ...
              >
                <div
                  className={`${
                    viewMode === "list" ? "w-20 h-24 flex-shrink-0" : "aspect-[3/4]"
                ...
                <div className={`${viewMode === "list" ? "flex-1" : "p-4"}`}>
                  <h3 className="font-semibold text-gray-200 mb-1 text-sm">{panel.manga}</h3>
                  <p
        ...
        <div className="fixed inset-0 bg-black bg-opacity-95 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-gray-900 rounded-2xl w-full max-w-6xl h-full max-h-[95vh] overflow-hidden shadow-2xl animate-scale-in border border-gray-700 flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-gray-700 bg-gradient-to-r from-gray-800 to-gray-900 flex-shrink-0">
              <div>
                <h3 className="text-xl font-bold text-gray-100">{selectedPanel.manga}</h3>
