...
export const PanelCard = ({ panel, viewMode, index, onClick }: PanelCardProps) => {
    return (
        <div
        key={panel.id}
        className={`group bg-black backdrop-blur-sm rounded-2xl shadow-xl
           overflow-hidden cursor-pointer transform transition-all duration-500 hover:scale-105
            hover:shadow-2xl border border-white-700/50 hover:border-gray-500/50 ${
            ...
            style={{
                ...
                <div className={`${viewMode === "list" ? "flex-1 text-left" : "p-4"}`}>
                    <div className="flex items-center gap-2 mb-2">
                        {/* Manga Indicator */}
                        <span className="text-xs font-semibold text-gray-400 bg-black px-2 py-1 rounded-full border border-gray-500">
                        {panel.manga}
                        </span>
                        {viewMode === "list" && (
                            <span className="text-xs text-gray-200">#{panel.id}</span>
                        )}
                    </div>
